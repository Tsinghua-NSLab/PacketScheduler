 ...
 *
 * There is no corresponding InputPort class, as the output port already forces
 * a rate limit. OutputPort's subclasses are forced to handle the enqueuing of
 * packets, and are allowed to drop packets depending on their own drop strategy
 * to handle congestion at the port (e.g. tail-drop, RED, ...).
 */
public abstract class OutputPort {
	 ...
	 * Constructor.
	 *
	 * @param ownNetworkDevice    Source network device to which this output port is
	 *                            attached
	 * @param targetNetworkDevice Target network device that is on the other side of
	 *                            the link
	 * @param link                Link that this output ports solely governs
	 * @param queue               Queue that governs how packet are stored queued in
	 *                            the buffer
	 */
	protected OutputPort(NetworkDevice ownNetworkDevice, NetworkDevice targetNetworkDevice, Link link,
			Queue<Packet> queue) {
	...
	/**
	 * Enqueue the given packet for sending. There is no guarantee that the packet
	 * is actually sent, as the queue buffer's limit might be reached.
	 *
	...
	/**
	 * To be used with SP-PIFO to control droppings There is no guarantee that the
	 * packet is actually sent, as the queue buffer's limit might be reached.
	 *
	 * @param packet Packet instance
	 */
	protected final void potentialEnqueue(Packet packet) {
		...
		if (!isSending) {
			...
			Simulator.registerEvent(
					new PacketDispatchedEvent(packet.getSizeBit() / link.getBandwidthBitPerNs(), packet, this));
	...
	/**
	 * To be used with PIFO to control droppings There is no guarantee that the
	 * packet is actually sent, as the queue buffer's limit might be reached. If the
	 * limit is reached, the packet with lower priority (higher rank) is dropped.
	 * 
	 * @param packet Packet instance
	 */
	...
	/**
	 * Called when a packet has actually been send completely. In response, register
	 * arrival event at the destination network device, and starts sending another
	 * packet if it is available.
	 *
	 * @param packet Packet instance that was being sent
	 */
	void dispatch(Packet packet) {

		// Finished sending packet, the last bit of the packet should arrive the
		// link-delay later
		if (!link.doesNextTransmissionFail(packet.getSizeBit())) {
			Simulator.registerEvent(new PacketArrivalEvent(link.getDelayNs(), packet, targetNetworkDevice));
		}
		...
		if (!queue.isEmpty()) {
			...
			Simulator.registerEvent(new PacketDispatchedEvent(
					packetFromQueue.getSizeBit() / link.getBandwidthBitPerNs(), packetFromQueue, this));
			// It is sending again
			isSending = true;
	...
	/**
	 * To be used with PIFO to control fairness. There is no guarantee that the
	 * packet is actually sent, as the queue buffer's limit might be reached. If the
	 * limit is reached, the packet with lower priority (higher rank) is dropped.
	 * 
	 * @param packet Packet instance
	...
	/**
	 * Return the network identifier of its own device (to which this output port is
	 * attached to).
	 *
	...
	public String toString() {
		return "OutputPort<" + ownId + " -> " + targetId + ", link: " + link + ", occupied: " + bufferOccupiedBits
	 ...
	 *
	 * NOTE: adapting the queue will most likely result in strange values in the
	 * port queue state log.
	 *
	 ...
	 *
	 * NOTE: adapting the buffer occupancy counter from your own implementation will
	 * most likely result in strange values in the port queue state log.
	 *
