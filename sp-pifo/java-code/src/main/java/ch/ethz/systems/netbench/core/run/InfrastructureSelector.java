...
import ch.ethz.systems.netbench.xpt.sppifo.ports.SPPIFOTryEqual.TEOutputPortGenerator;
...
class InfrastructureSelector {
	...
	/**
	 * Select the network device generator, which, given its identifier, generates
	 * an appropriate network device (possibly with transport layer).
	 *
	 * Selected using following properties: network_device=...
	...
	static NetworkDeviceGenerator selectNetworkDeviceGenerator() {
		...
		default:
			throw new PropertyValueInvalidException(Simulator.getConfiguration(), "network_device_intermediary");
		...
		case "forwarder_switch":
			return new ForwarderSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		case "ecmp_switch":
			return new EcmpSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		case "random_valiant_ecmp_switch":
			return new RangeValiantSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		case "ecmp_then_random_valiant_ecmp_switch":
			return new EcmpThenValiantSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		case "source_routing_switch":
			return new SourceRoutingSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		case "ecmp_then_source_routing_switch":
			return new EcmpThenSourceRoutingSwitchGenerator(intermediaryGenerator,
					Simulator.getConfiguration().getGraphDetails().getNumNodes());

		default:
			throw new PropertyValueInvalidException(Simulator.getConfiguration(), "network_device");
	...
	/**
	 * Select the link generator which creates a link instance given two directed
	 * network devices.
	 *
	 * Selected using following property: link=...
	 *
	...
	static LinkGenerator selectLinkGenerator() {
		...
		case "perfect_simple":
			return new PerfectSimpleLinkGenerator(Simulator.getConfiguration().getLongPropertyOrFail("link_delay_ns"),
					Simulator.getConfiguration().getLongPropertyOrFail("link_bandwidth_bit_per_ns"));

		case "split_bw":
			return new SplitBandwidthLinkGenerator(Simulator.getConfiguration().getLongPropertyOrFail("link_delay_ns"),
					Simulator.getConfiguration().getLongPropertyOrFail("link_bandwidth_bit_per_ns"));

		default:
			throw new PropertyValueInvalidException(Simulator.getConfiguration(), "link");
	 ...
	 *
	 * Selected using following property: output_port=...
	 *
	...
	static OutputPortGenerator selectOutputPortGenerator() {
		...
		default:
			throw new PropertyValueInvalidException(Simulator.getConfiguration(), "output_port");
	...
	static TransportLayerGenerator selectTransportLayerGenerator() {
		...
		default:
			throw new PropertyValueInvalidException(Simulator.getConfiguration(), "transport_layer");
